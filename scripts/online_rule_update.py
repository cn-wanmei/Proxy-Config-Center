...
def valid(v):
    ...
    except ValueError: return bool(re.fullmatch(r'(?:[a-z0-9](?:[a-z0-9-]{0,61}[a-z0-9])?\.)+[a-z]{2,63}',v))
...
def extract_list(text):
    ...
    for line in text.splitlines():
        ...
        if m:
            ...
            try: cidrs.add(str(ipaddress.ip_network(v,strict=False))); continue
            except ValueError: pass
            if valid(v): domains.add(v.rstrip('.'))
            continue
        for v in HOST_RE.findall(line):
            if valid(v): domains.add(v.lower().rstrip('.'))
    return domains,cidrs

def extract_m365(text):
    ...
    for item in data if isinstance(data,list) else []:
        for v in item.get('urls',[]) or []:
            ...
            if valid(v):domains.add(v)
        for v in item.get('ips',[]) or []:
            try:cidrs.add(str(ipaddress.ip_network(v,strict=False)))
            except ValueError:pass
    return domains,cidrs
...
def extract_github(text):
    ...
    domains,cidrs=set(),set()
    dmap=data.get('domains',{}) if isinstance(data,dict) else {}
    if isinstance(dmap,dict):
        for values in dmap.values():
            for v in values or []:
                v=str(v).lower().lstrip('*.')
                if valid(v):domains.add(v)
    if isinstance(data,dict):
        for key,values in data.items():
            if key=='domains' or not isinstance(values,list):continue
            for v in values:
                try:cidrs.add(str(ipaddress.ip_network(v,strict=False)))
                except (ValueError,TypeError):pass
    return domains,cidrs

def classify(source,value):
    value=value.lower()
    for service,needles in CLASSIFIERS.get(source,{}).items():
        if any(value==n or value.endswith('.'+n) for n in needles):return service
    return source
...
def merge(doc,domains,cidrs):
    changed=False
    for rule in doc.get('rules',[]):
        typ=rule.get('type')
        if typ=='domain-suffix' and domains:
            old={str(x).lower() for x in rule.get('values',[])}; new=sorted(old|domains)
            if new!=rule.get('values',[]):rule['values']=new;changed=True
        if typ in ('ip-cidr','ip-cidr6') and cidrs:
            old=set(rule.get('values',[]));new=sorted(old|{x for x in cidrs if (':' in x)==(typ=='ip-cidr6')})
            if new!=rule.get('values',[]):rule['values']=new;changed=True
...
def run(dry=False):
    ...
    for feed in cfg.get('feeds',[]):
        ...
        try:
            text=fetch(url)
            if source=='microsoft':domains,cidrs=extract_m365(text)
            elif source=='github':domains,cidrs=extract_github(text)
            else:domains,cidrs=extract_list(text)
        ...
        for value in domains|cidrs:
            service=classify(source,value)
            bucket=observed.setdefault(service,{'domains':set(),'cidrs':set()})
            bucket['cidrs' if '/' in value else 'domains'].add(value)
    if not observed:raise SystemExit('all online sources failed; refusing changes')
    changed=set()
    for service,bucket in observed.items():
        path=SERVICES/f'{service}.yaml'
        if not path.exists():continue
        doc=yaml.safe_load(path.read_text())
        if merge(doc,bucket['domains'],bucket['cidrs']):changed.add(path)
        owned=state.setdefault('owned',{}).setdefault(service,[])
        state['owned'][service]=sorted(set(owned)|bucket['domains']|bucket['cidrs'])
        for value in bucket['domains']|bucket['cidrs']:state.setdefault('misses',{}).pop(f'{service}:{value}',None)
    ...
    for service,owned in list(state.get('owned',{}).items()):
        bucket=observed.get(service,{'domains':set(),'cidrs':set()});current=bucket['domains']|bucket['cidrs'];path=SERVICES/f'{service}.yaml'
        if not path.exists():continue
        doc=yaml.safe_load(path.read_text())
        for value in list(owned):
            ...
            for rule in doc.get('rules',[]):
                ...
                if value in values:rule['values']=[x for x in values if x!=value];removed=True
            if removed:changed.add(path)
            state['owned'][service]=[x for x in state['owned'][service] if x!=value]
        if not dry and path in changed:path.write_text(yaml.safe_dump(doc,sort_keys=False,allow_unicode=True))
...
if __name__=='__main__':
    parser=argparse.ArgumentParser();parser.add_argument('--dry-run',action='store_true');run(parser.parse_args().dry_run)
