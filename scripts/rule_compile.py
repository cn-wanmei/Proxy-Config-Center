...
def _canonical_rule(rule: dict) -> dict:
    from engines.semantic import content_sha256, global_rule_id

    rtype = rule['type']
    value = rule['value']
    return {
        'rule_id': rule['rule_id'],
        'global_rule_id': global_rule_id(rtype, value),
        'sha256': rule.get('content_hash') or content_sha256(rtype, value),
        'type': rtype,
        'value': value,
...
def compile_rules(out: Path):
    ...
    for rule in result.kept:
        policy_id = _safe_policy_id(str(rule['policy_id']))
        policies.setdefault(policy_id, []).append(rule)

    emitted = []
    for policy_id in sorted(policies):
        canonical = [_canonical_rule(rule) for rule in policies[policy_id]]
        items = sorted(
            {rule['rule_id']: rule for rule in canonical}.values(),
            key=lambda rule: (rule['priority'], rule['type'], rule['value'], rule['rule_id']),
        )
        payload = {
            'schema_version': 4,
            'policy_id': policy_id,
