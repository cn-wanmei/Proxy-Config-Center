...
def _is_ip_or_cidr(value: str) -> bool:
    try: ipaddress.ip_network(value,strict=False) if "/" in value else ipaddress.ip_address(value); return True
...
def classify_pair(a:Mapping[str,Any],b:Mapping[str,Any])->str|None:
    ...
    if at==bt and av==bv:
        if a.get("policy_id")==b.get("policy_id"): return "duplicate"
        return "conflict" if int(a.get("priority",500))==int(b.get("priority",500)) else "overlap"
...
def analyze(rules:Sequence[Mapping[str,Any]])->Dict[str,Any]:
    ...
    for (rtype,value),idxs in sorted(exact.items()):
        if len(idxs)>1:
            policies=sorted({normalized[i]["policy_id"] for i in idxs}); priorities={normalized[i]["priority"] for i in idxs}
            kind="duplicate" if len(policies)==1 else ("conflict" if len(priorities)==1 else "overlap")
            findings.append({"kind":kind,"type":rtype,"value":value,"policies":policies,"priorities":sorted(priorities),"rule_ids":[normalized[i]["rule_id"] for i in idxs]})
    for i,child in enumerate(normalized):
        ...
        for cut in range(1,len(labels)):
            for pi in suffix.get(".".join(labels[cut:]),[]):
                ...
                if rel in ("shadow","overlap"):
                    findings.append({"kind":rel,"parent":parent["rule_id"],"child":child["rule_id"],"parent_policy":parent["policy_id"],"child_policy":child["policy_id"],"parent_value":parent["value"],"child_value":child["value"],"parent_priority":parent["priority"],"child_priority":child["priority"]})
    validation=[]
    for r in normalized:
