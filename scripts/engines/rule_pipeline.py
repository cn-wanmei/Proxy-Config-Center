...
def run_pipeline():
    atoms=[a.to_dict() for a in collect_atoms()]; semantic=analyze(atoms); pollution=detect_pollution([type("A",(),a)() for a in []]) if False else detect_pollution(collect_atoms()); anomalies=detect_source_count_anomaly(collect_atoms()); errors=[]; warnings=[]
...
def write_pipeline_artifacts(result,out:Path,version:str):
    out.mkdir(parents=True,exist_ok=True); report={"schema_version":1,"compiler_version":version,"ok":result.ok,"summary":{"atoms":len(result.atoms),"kept":len(result.kept),"errors":len(result.errors),"warnings":len(result.warnings),**result.semantic["summary"]},"errors":result.errors,"warnings":result.warnings,"semantic":result.semantic,"pollution":result.pollution,"source_anomalies":result.anomalies}; (out/"rule-audit.json").write_text(json.dumps(report,ensure_ascii=False,indent=2,sort_keys=True)+"\n",encoding="utf-8")
    manifest={"schema_version":1,"version":version,"rule_count":len(result.kept),"rules":[{"rule_id":r["rule_id"],"global_rule_id":r["global_rule_id"],"sha256":r["sha256"],"policy_id":r["policy_id"],"type":r["type"],"value":r["value"],"source_file":r.get("source_file",""),"provenance":r.get("provenance","")} for r in result.kept]}; (out/"rule-manifest.json").write_text(json.dumps(manifest,ensure_ascii=False,indent=2,sort_keys=True)+"\n",encoding="utf-8")
